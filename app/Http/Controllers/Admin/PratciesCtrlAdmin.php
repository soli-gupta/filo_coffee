...
class PratciesCtrlAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder = 'practices';
    }
    ...
    public function index()
    {
        ...
        if (isset($GET_DATA['delete'])) {
            $delete_status = PratcieModel::find($GET_DATA['delete'])->delete();
        ...
        if (isset($GET_DATA['sorting'])  &&  isset($GET_DATA['orderBy'])  && $GET_DATA['sorting'] != ''  && $GET_DATA['orderBy'] != '') {
            ...
            $data_rows = PratcieModel::orderBy($sorting, $orderBy);
        } else {
            $data_rows = PratcieModel::orderBy('created_at', 'desc');
        }
        //$data_rows=PratcieModel::orderBy('id', 'desc');
        ...
        if (isset($request->formAction2)) {
            ...
            if (count($ids) > 0) {
                if ($formAction == 'status-1') {
                    PratcieModel::whereIn('id',  $ids)->update(['status' => '1']);
                    ...
                    PratcieModel::whereIn('id',  $ids)->update(['status' => '2']);
                    $message_type = "message_susuccess";
    ...
    public function view($id)
    {
        ...
        $attributes = PratcieModel::get();
        $this->roleChecking();
        $data = PratcieModel::where('id', $id)->first();
        if ($data) {
            $page_array = array(
    ...
    public function save(Request $request)
    {
        ...
        if ($POST_DATA['submit_type'] == '3' && $id != '') {
            $delete_status = PratcieModel::find($id)->delete();
            $message_type = "message_susuccess";
            $message_text = "Record has been deleted successfully!";
            return redirect(ADMIN_FOLDER . '/' . $this->name_url_folder)->with($message_type, $message_text);
        }
