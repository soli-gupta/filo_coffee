...
class LocationCtrlAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder = 'locations';
    }
    ...
    public function index()
    {
        ...
        if (isset($GET_DATA['delete'])) {
            $delete_status = LocationModel::find($GET_DATA['delete'])->delete();
        ...
        if (isset($GET_DATA['sorting'])  &&  isset($GET_DATA['orderBy'])  && $GET_DATA['sorting'] != ''  && $GET_DATA['orderBy'] != '') {
            ...
            $data_rows = LocationModel::orderBy($sorting, $orderBy);
        } else {
            $data_rows = LocationModel::orderBy('created_at', 'desc');
        }
        //$data_rows=LocationModel::orderBy('id', 'desc');
        ...
        if (isset($request->formAction2)) {
            ...
            if (count($ids) > 0) {
                if ($formAction == 'status-1') {
                    LocationModel::whereIn('id',  $ids)->update(['status' => '1']);
                    ...
                    LocationModel::whereIn('id',  $ids)->update(['status' => '2']);
                    $message_type = "message_susuccess";
    ...
    public function save(Request $request)
    {
        ...
        if ($POST_DATA['submit_type'] == '3' && $id != '') {
            $delete_status = LocationModel::find($id)->delete();
            $message_type = "message_susuccess";
            $message_text = "Record has been deleted successfully!";
            return redirect(ADMIN_FOLDER . '/' . $this->name_url_folder)->with($message_type, $message_text);
        }
        ...
        try {
            ...
            if ($id == '') {
                $save = LocationModel::create($POST_DATA);
            } else {


                $save = LocationModel::find($id);
                $save->fill($POST_DATA);
            }
    ...
    public function export(Request $request)
    {
        ...
        if (count($data_rows) > 0) {
            ...
            $export_data = "Id,Name,Sorting,Date,UpdatedAt \n";
