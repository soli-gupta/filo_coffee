...
class ProductSubCategoryCtrlAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user =  Auth::user();
        $this->name_url_folder = 'product-sub-category';
    }
    ...
    public function index()
    {
        ...
        if (isset($GET_DATA['delete'])) {
            $delete_status = ProductSubCategoryModel::find($GET_DATA['delete'])->delete();
        ...
        $category_type = ProductCategoryModel::where('status', 1)->orderBy('sorting', 'asc')->get();
        $data_rows = ProductSubCategoryModel::orderBy('id', 'desc');
    ...
    public function view($id)
    {
        ...
        $this->roleChecking();
        $data = ProductSubCategoryModel::where('id', $id)->first();
        if ($data) {
            $page_array = array(
                'title' => 'View ',
                'page_name' => $this->name_url_folder,
    ...
    public function save(Request $request)
    {
        ...
        if ($POST_DATA['submit_type'] == '3' && $id != '') {
            $delete_status = ProductSubCategoryModel::find($id)->delete();
            $message_type = "message_susuccess";
            $message_text = "Record has been deleted successfully!";
            return redirect(ADMIN_FOLDER . '/' . $this->name_url_folder)->with($message_type, $message_text);
        }
        ...
        if ($id) {
            $array_validate['slug'] = "required|unique:product_sub_category,slug,$id";
        } else {
            $array_validate['slug'] = "required|unique:product_sub_category|max:255";
        }
        ...
        try {
            ...
            if ($id == '') {
                $save = ProductSubCategoryModel::create($POST_DATA);
            } else {
                $save = ProductSubCategoryModel::find($id);
                $save->fill($POST_DATA);
            }
    ...
    public function getCategoryById(Request $request)
    {
        ...
        $sub_cate = ProductSubCategoryModel::where("cate_id", $category_id)->get();
