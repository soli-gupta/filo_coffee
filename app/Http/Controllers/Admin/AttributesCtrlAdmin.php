...
class AttributesCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'attributes';
    }
...
public function roleChecking() {
  ...
  $this->roleChecking(); 
    ...
    if(isset($GET_DATA['delete'])){
        $delete_status = AttributesModel::find($GET_DATA['delete'])->delete();
...
if(isset($GET_DATA['sorting'])  &&  isset($GET_DATA['orderBy'])  && $GET_DATA['sorting']!=''  && $GET_DATA['orderBy']!='')
        { 
           ...
           $data_rows=AttributesModel::orderBy($sorting, $orderBy);
        } else {
           $data_rows=AttributesModel::orderBy('created_at', 'desc');
        }
 ...
 //$data_rows=AttributesModel::orderBy('id', 'desc');
...
//die("aa");
      ...
      $this->roleChecking(); 
        ...
        if(isset($request->formAction2))
        {
            ...
            if(count($ids)>0)
            {
                ...
                {
                    AttributesModel::whereIn('id',  $ids)->update(['status' => '1']);
                ...
                {
                    AttributesModel::whereIn('id',  $ids)->update(['status' => '2']);
                     $message_type="message_susuccess";
  ...
  $attributes = AttributesModel::get();
    ...
    $this->roleChecking(); 
    $data = AttributesModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
   ...
   if($POST_DATA['submit_type']=='3' && $id!=''){
        $delete_status = AttributesModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=AttributesModel::create($POST_DATA);

                    } else {
                               
            
                $save=AttributesModel::find($id);              
                $save->fill($POST_DATA);
      ...
      $this->roleChecking(); 
        ...
        $tot_record_found=0;
           if(count($data_rows)>0){
            ...
            $export_data="Id,Attribute,type,Sorting,Date,UpdatedAt \n";
