...
class ServicesCtrlAdmin extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($GET_DATA['delete'])) {
            $delete_status = ServicesModel::find($GET_DATA['delete'])->delete();
        ...
        if (isset($GET_DATA['sorting'])  &&  isset($GET_DATA['orderBy'])  && $GET_DATA['sorting'] != ''  && $GET_DATA['orderBy'] != '') {
            ...
            $data_rows = ServicesModel::orderBy($sorting, $orderBy);
        } else {
            $data_rows = ServicesModel::orderBy('created_at', 'desc');
        }
        //$data_rows=ServicesModel::orderBy('id', 'desc');
        ...
        if (isset($request->formAction2)) {
            ...
            if (count($ids) > 0) {
                if ($formAction == 'status-1') {
                    ServicesModel::whereIn('id',  $ids)->update(['status' => '1']);
                    ...
                    ServicesModel::whereIn('id',  $ids)->update(['status' => '2']);
                    $message_type = "message_susuccess";
    ...
    public function save(Request $request)
    {
        ...
        if ($POST_DATA['submit_type'] == '3' && $id != '') {
            $delete_status = ServicesModel::find($id)->delete();
            $message_type = "message_susuccess";
            $message_text = "Record has been deleted successfully!";
            return redirect(ADMIN_FOLDER . '/' . $this->name_url_folder)->with($message_type, $message_text);
        }
