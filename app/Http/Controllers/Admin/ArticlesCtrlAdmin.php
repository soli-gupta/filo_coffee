...
class ArticlesCtrlAdmin extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($GET_DATA['delete'])) {
            $delete_status = ArticlesModel::find($GET_DATA['delete'])->delete();
        ...
        $data_rows = ArticlesModel::orderBy('posted_date', 'desc');
    ...
    public function view($id)
    {
        ...
        $this->roleChecking();
        $data = ArticlesModel::where('id', $id)->first();
        if ($data) {
            $page_array = array(
                'title' => 'View Articles',
                'page_name' => $this->name_url_folder,
    ...
    public function save(Request $request)
    {
        ...
        if ($POST_DATA['submit_type'] == '3' && $id != '') {
            $delete_status = ArticlesModel::find($id)->delete();
            $message_type = "message_susuccess";
            $message_text = "Record has been deleted successfully!";
            return redirect(ADMIN_FOLDER . '/' . $this->name_url_folder)->with($message_type, $message_text);
        }
        ...
        try {
            ...
            if ($uploadedFile) {
                $destinationPath = 'media/articles/image/';
            ...
            if ($uploadedFile) {
                $destinationPath = 'media/articles/image2/';
            ...
            if ($id == '') {
                $save = ArticlesModel::create($POST_DATA);
            } else {
                $save = ArticlesModel::find($id);
                $save->fill($POST_DATA);
            }
    ...
    public function export(Request $request)
    {
        ...
        $data_rows = ArticlesModel::orderBy('id', 'desc');
        $data_rows = $this->getFilter($data_rows);
